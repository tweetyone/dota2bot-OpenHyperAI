...
function X.ConsiderShurikenToss()
	...
	local nCastRange = Fu.GetProperCastRange(false, bot, ShurikenToss:GetCastRange())
	...
	then
		...
		then
			...
			then
				for _, nUnit in pairs( nEnemyUnitList )
				do
					if Fu.IsInRange( nUnit, npcEnemy, nRadius - 100 )
						and not nUnit:IsMagicImmune()
						and Fu.CanCastOnTargetAdvanced( nUnit )
...
function X.ConsiderTrack()
	...
	local nCastRange = Fu.GetProperCastRange(false, bot, Track:GetCastRange())
	local nInRangeEnemyList = Fu.GetAroundEnemyHeroList( nCastRange + 100 )
...
function X.ConsiderFriendlyShadow()
	...
	local nCastRange = Fu.GetProperCastRange(false, bot, FriendlyShadow:GetCastRange())
