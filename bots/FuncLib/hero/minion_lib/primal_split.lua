...
function AttackUnits(hMinionUnit)
	...
	if enemies ~= nil and #enemies >= 1 then
		target = Fu.GetWeakestUnit(enemies)
	end
...
function UseCyclone(hMinionUnit)
	...
	for v, enemy in pairs(enemies) do
		if Fu.IsValidTarget(enemy) and enemy:IsChanneling() and Fu.IsNotImmune(enemy) then
			target = enemy
			break
		end
	end
	...
	if target == nil and #filteredenemies >= 2 then
		target = Fu.GetStrongestEnemyHero(filteredenemies)
	end
...
function UseHurlBoulder(hMinionUnit)
	...
	local enemies = hMinionUnit:GetNearbyHeroes(CastRange + 500, true, BOT_MODE_NONE)
	local target = Fu.GetWeakestUnit(enemies)
...
function ConsiderMove(hMinionUnit)
    ...
    local loc = Fu.GetClosestTeamLane(hMinionUnit)
    local nInRangeEnemy = Fu.GetEnemiesNearLoc(loc, 1600)
    if #nInRangeEnemy == 0
    then
        return BOT_ACTION_DESIRE_HIGH, loc
    else
        if nAllyHeroes and Fu.IsValidHero(nAllyHeroes[1])
        then
