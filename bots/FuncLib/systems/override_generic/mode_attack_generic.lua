...
function Generic.GetDesire()
	...
	local tAllyHeroes_real = Fu.GetAlliesNearLoc(botLocation, 1600)
	local tEnemyHeroes = bot:GetNearbyHeroes(1600, true, BOT_MODE_NONE)
	local tEnemyHeroes_real = Fu.GetEnemiesNearLoc(botLocation, 1600)
	...
	if Fu.IsInLaningPhase() then
		...
		for _, creep in pairs(tEnemyLaneCreeps) do
			if Fu.IsValid(creep) and Fu.IsInRange(bot, creep, 600) and creep:GetAttackTarget() == bot then
	...
	for _, enemyHero in ipairs(unitList_enemies) do
		...
		and (Fu.IsInLaningPhase() and Fu.IsInRange(bot, enemyHero, 1600)
			or (not Fu.IsInLaningPhase() and (((GetUnitToUnitDistance(bot, enemyHero) - botAttackRange) / bot:GetCurrentMovementSpeed()) <= 6.0)))
		then
			fAllyDamage = 0
			...
			for _, allyHero in ipairs(unitList_allies) do
				...
				and (bot:GetPlayerID() == allyHero:GetPlayerID()
					...
					or Fu.IsInRange(allyHero, enemyHero, botAttackRange + 200))
				then
			...
			for _, possibleEnemy in ipairs(unitList_enemies) do
				if Fu.IsValidHero(possibleEnemy)
				and Fu.GetHP(possibleEnemy) >= 0.25
				and not Fu.IsSuspiciousIllusion(possibleEnemy)
			...
			if Fu.IsValidBuilding(nEnemyTowersNear[1]) then
			...
			if b1 or b2 or b3 then
				...
				if dist <= 2000 or ((dist / bot:GetCurrentMovementSpeed()) <= 10.0) then
					if Fu.IsInLaningPhase() and bot:GetActiveMode() == BOT_MODE_ATTACK then
						if bot:WasRecentlyDamagedByTower(2.0) or (Fu.IsValidBuilding(tEnemyTowers[1]) and tEnemyTowers[1]:GetAttackTarget() == bot) then
	...
	if not bCore or not Fu.IsInLaningPhase() then
		for _, allyHero in ipairs(unitList_allies) do
			if bot ~= allyHero
			and Fu.IsValidHero(allyHero)
			and Fu.IsInRange(bot, allyHero, 4000)
			and not Fu.IsSuspiciousIllusion(allyHero)
			and not allyHero:HasModifier('modifier_necrolyte_reapers_scythe')
			and not allyHero:HasModifier('modifier_skeleton_king_reincarnation_scepter_active')
			...
			then
				...
				local bWeAreStronger_Ally = Fu.WeAreStronger(allyHero, 1200)

				if Fu.IsValidHero(enemyHero)
				and Fu.IsInRange(allyHero, enemyHero, 1600)
				and not Fu.IsSuspiciousIllusion(enemyHero)
				and not enemyHero:HasModifier('modifier_necrolyte_reapers_scythe')
				then
					local nInRangeAlly = Fu.GetAlliesNearLoc(enemyHero:GetLocation(), 1600)
					local nInRangeEnemy = Fu.GetEnemiesNearLoc(enemyHero:GetLocation(), 1600)
		...
		for _, id in ipairs(GetTeamPlayers(GetOpposingTeam())) do
			if IsHeroAlive(id) and id == botTarget.id then
				...
				if info then
					...
					and Fu.GetDistance(dInfo.location, botTarget.location) <= 1200 then
						local tAllyHeroes_real = Fu.GetAlliesNearLoc(botLocation, 1600)
						local tEnemyHeroes_real2 = Fu.GetEnemiesNearLoc(botLocation, 1600)
...
function Generic.Think()
	...
	local bTeamFight = Fu.IsInTeamFight(bot, 1600)

	-- Pugna life drain: run away
	if bot:HasModifier('modifier_pugna_life_drain') then
		for _, enemy in ipairs(nEnemyHeroes) do
			if Fu.IsValidHero(enemy) and Fu.IsInRange(bot, enemy, 750) and not Fu.IsSuspiciousIllusion(enemy)
	...
	elseif bot:HasModifier('modifier_razor_static_link_debuff') then
		...
		if Fu.IsValidBuilding(nEnemyTowers[1]) and nEnemyTowers[1]:GetAttackTarget() == bot and not bTeamFight then
			local unitDamage = 0
			for _, unit in pairs(GetUnitList(UNIT_LIST_ENEMIES)) do
				if Fu.IsValid(unit) and unit:GetAttackTarget() == bot then
			...
			if unitDamage / (botHealth + bot:GetHealthRegen() * 3.0) >= 0.2 then
				bot:Action_MoveToLocation(Fu.VectorAway(botLocation, nEnemyTowers[1]:GetLocation(), 800))
				return
			end
		end
	end
	...
	for _, enemy in pairs(nEnemyHeroes) do
		...
		and Fu.IsInRange(bot, enemy, 1200)
		...
		then
			...
			if enemyName ~= 'npc_dota_hero_bristleback' then
				if Fu.IsCore(enemy) then mul = mul * 1.5 else mul = mul * 0.5 end
			end

			if (Fu.IsEarlyGame() or Fu.IsMidGame()) and Fu.IsValidBuilding(nEnemyTowers[1]) and Fu.IsInRange(enemy, nEnemyTowers[1], 800) then
				mul = mul * 0.5
			end

			local nAllyHeroes_Attacking = Fu.GetSpecialModeAllies(enemy, 1200, BOT_MODE_ATTACK)
			local nInRangeAlly = Fu.GetAlliesNearLoc(enemy:GetLocation(), 900)
			local nInRangeEnemy = Fu.GetEnemiesNearLoc(enemy:GetLocation(), 900)

			local enemyScore = (math.min(1, bot:GetAttackRange() / GetUnitToUnitDistance(bot, enemy)))
				* ((1 - Fu.GetHP(enemy)) * Fu.GetTotalEstimatedDamageToTarget(nAllyHeroes_Attacking, enemy, 5.0))
				* mul
	...
	if __target == nil then
		__target = Fu.GetAttackableWeakestUnit(bot, 1200, true, true)
	end

	if __target and Fu.IsValidHero(__target) then
