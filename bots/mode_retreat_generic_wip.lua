...
local targetBot = Fu.GetProperTarget( bot )
...
function GetDesire()
    ...
    if Fu.GetHP(bot) > 0.2 and (bot:IsUsingAbility() or bot:IsChanneling()) then return BOT_ACTION_DESIRE_NONE end

	-- 有特殊增益状态不要跑
	if Fu.IsHaveAegis( bot )
	...
	or Fu.GetModifierTime(bot, "modifier_muerta_pierce_the_veil_buff") > 0.5
	or (Fu.GetModifierTime(bot, 'modifier_dazzle_shallow_grave') > 0.5 and Fu.GetHP(bot) > 0.2)
	or Fu.GetModifierTime(bot, 'modifier_oracle_false_promise_timer') > 0.5
    ...
    nAllyHeroes = Fu.GetNearbyHeroes(bot, 1400, false, BOT_MODE_NONE)
    nEnemyHeroes = Fu.GetNearbyHeroes(bot, 1400, true, BOT_MODE_NONE)
	targetBot = Fu.GetProperTarget( bot )
	...
	if Fu.IsInLaningPhase() and Utils.RecentlyTookDamage(bot, 2) then
	...
	if #nEnemyHeroes > 0 then
		for _, hero in pairs(nEnemyHeroes) do
			...
			then
				...
				if Fu.IsAttacking( hero ) and hero:IsFacingLocation(bot:GetLocation(), 5) then
	...
	-- if Fu.IsLaning( bot ) or bot:GetLevel() <= 10 then
		if not weAreStronger and Fu.GetHP(bot) < 0.7 then
		...
		for _, enemy in pairs(nEnemyHeroes) do
			...
			and Fu.GetHP(enemy) > Fu.GetHP(bot) - 0.2 then
				return BOT_ACTION_DESIRE_VERYHIGH
			end
		end
	-- end
	...
	if #towers >= 1 then
		...
		if towerType == TOWER_TOP_1 or towerType == TOWER_MID_1 or towerType == TOWER_BOT_1 then
			...
			and Fu.GetHP(targetBot) > 0.2
			and not (bot:IsStunned() or bot:IsHexed() or Fu.IsInRange(bot, targetBot, bot:GetAttackRange())) then
	...
	-- 别轻易上高送
	if #nAllyHeroes <= 2 and #nAllyHeroes + 1 <= Fu.GetNumOfAliveHeroes(true)
	and GetUnitToLocationDistance(bot, Fu.GetEnemyFountain()) < 5500
	...
	-- 掩护队友
	if #nAllyHeroes >= 2 and Fu.GetHP(bot) > 0.3 then
		for _, hero in pairs(nAllyHeroes) do
			if hero ~= bot and Fu.IsRetreating(hero) and Fu.GetHP(hero) < Fu.GetHP(bot) then
				return BOT_ACTION_DESIRE_NONE
			end
		end
	end
	...
	-- elseif #uniqueMates < #uniqueEnemies or (targetBot ~= nil and Fu.GetHP(targetBot) > Fu.GetHP(bot)) then
    --     return BOT_ACTION_DESIRE_HIGH
	-- end
	...
	and (not Fu.IsValid(targetBot) or Fu.GetHP(targetBot) > Fu.GetHP(bot) or Utils.RecentlyTookDamage(bot, 3))
	then
		return BOT_ACTION_DESIRE_VERYHIGH
	end
	...
	-- if Fu.IsValidHero(closestEnemy)
	...
	retreatDesire = retreatDesire + RemapValClamped(Fu.GetHP(bot), 0.7, 0, BOT_ACTION_DESIRE_NONE, BOT_ACTION_DESIRE_ABSOLUTE )
	possibleMaxDesire = BOT_ACTION_DESIRE_VERYHIGH * 4

	if Fu.IsValid(closestEnemy) and Fu.GetHP(bot) < Fu.GetHP(closestEnemy) then
