...
if Fu.Role.IsPvNMode() or Fu.Role.IsAllShadow() then X['sBuyList'], X['sSellList'] = { 'PvN_TA' }, {} end
...
function X.SkillsComplement()
	...
	if Fu.CanNotUseAbility( bot ) or bot:HasModifier( 'modifier_templar_assassin_meld' ) then return end
	...
	then
		...
		midLoc = Fu.GetUnitTowardDistanceLocation( bot, myMidTower1, 928 )
...
function X.ConsiderQ()
		...
		then
			...
			do
				if Fu.IsValid( creep )
					...
					and Fu.IsInRange( bot, creep, nRange + 300 )
					and Fu.CanKillTarget( creep, nTotalDamage, nDamageType )
				then
					return BOT_ACTION_DESIRE_MODERATE
				end
			end
	...
	--格挡弹道
	if Fu.IsNotAttackProjectileIncoming( bot, 1600 )
		or Fu.GetAttackProjectileDamageByRange( bot, 1600 ) >= 110
	then
		return BOT_ACTION_DESIRE_MODERATE
	end
	...
	then
		...
		if #nLaneCreeps >= 2
		then
			local targetCreep = Fu.GetMostHpUnit( nLaneCreeps )
	...
	--发育时对野怪输出
	if Fu.IsFarming( bot ) and nSkillLV >= 2
	...
	then
		...
		then
			...
			if Fu.IsValid( targetCreep )
				...
				and Fu.IsInRange( targetCreep, bot, nRange + 50 )
			then
				return BOT_ACTION_DESIRE_HIGH
			end
		end
	...
	if Fu.IsGoingOnSomeone( bot ) and nSkillLV >= 2
	then
		if Fu.IsValidHero( botTarget )
			and Fu.CanBeAttacked( botTarget )
			and Fu.IsInRange( botTarget, bot, 2000 )
		then
			return BOT_ACTION_DESIRE_HIGH
		end
	end
	...
	then
		...
		if ( Fu.IsRoshan( npcTarget ) and Fu.GetHP( npcTarget ) > 0.3 and Fu.IsInRange( npcTarget, bot, nRange ) )
		then
	...
	then
		...
		if #tableNearbyEnemyHeroes > 0
			...
			or ( Fu.IsInEnemyArea( bot ) and nMP > 0.95 )
...
function X.ConsiderW()
	...
	local proDmg = Fu.GetAttackProjectileDamageByRange( bot, 1600 )
	...
	if Fu.IsRunning( bot ) then return BOT_ACTION_DESIRE_NONE	end
	...
	then
		local npcTarget = bot:GetAttackTarget()
		if ( Fu.IsRoshan( npcTarget ) and Fu.IsInRange( npcTarget, bot, nCastRange + 40 ) )
		then
			return BOT_ACTION_DESIRE_LOW
		end
	end
	...
	then
		...
		if Fu.IsValidHero( npcTarget )
			...
			and Fu.IsInRange( npcTarget, bot, nCastRange + 64 )
		then
			return BOT_ACTION_DESIRE_HIGH
		end
		...
		if Fu.IsValidHero( npcTarget )
			and Fu.CanBeAttacked( npcTarget )
			and Fu.IsInRange( npcTarget, bot, nCastRange + 24 )
		then
			return BOT_ACTION_DESIRE_HIGH
		end
	end
	...
	then
		...
		if Fu.IsValid( targetCreep )
			and Fu.IsInRange( targetCreep, bot, nCastRange + 50 )
	...
	then
		...
		then
			...
			if Fu.IsValid( targetCreep )
				and Fu.IsInRange( targetCreep, bot, nCastRange + 50 )
				and not targetCreep:HasModifier( "modifier_fountain_glyph" )
				and not Fu.CanKillTarget( targetCreep, nAttackDamage * 1.2, nDamageType )
				and ( Fu.CanKillTarget( targetCreep, nTotalDamage * 1.2, nDamageType )
	...
	local targetCreep = bot:GetAttackTarget()
	if Fu.IsValid( targetCreep )
		and Fu.IsKeyWordUnit( "ranged", targetCreep )
		and not targetCreep:HasModifier( "modifier_fountain_glyph" )
		and Fu.GetHP( targetCreep ) > 0.48
		and Fu.IsInRange( targetCreep, bot, nCastRange + 40 )
		and not Fu.CanKillTarget( targetCreep, nAttackDamage * 1.2, nDamageType )
		and Fu.CanKillTarget( targetCreep, nTotalDamage * 1.2, nDamageType )
	then
		return BOT_ACTION_DESIRE_HIGH
	...
	then
		...
		if Fu.IsValid( nAttackTarget )
			and Fu.IsInRange( nAttackTarget, bot, nCastRange + 50 )
			and not Fu.CanKillTarget( nAttackTarget, nAttackDamage * 3.28, nDamageType )
		then
			return BOT_ACTION_DESIRE_HIGH
		end
	end
...
function X.ConsiderR()
	...
	local enemyHeroes = Fu.GetNearbyHeroes(bot, 600, true, BOT_MODE_NONE )
	...
	do
		...
		then
			local npcTarget = Fu.GetProperTarget( member )
			if Fu.IsValidHero( npcTarget )
				and Fu.IsRunning( npcTarget )
				and Fu.IsInRange( npcTarget, bot, nCastRange + 800 )
				and not Fu.IsInRange( npcTarget, bot, bot:GetAttackRange() )
