...
local ____aba_buff = require(GetScriptDirectory().."/FuncLib/data/buff")
...
local role = Fu.Item.GetRoleItemsBuyList(bot)
...
local allAbilitiesList = Fu.Skill.GetAbilityList(bot)
...
local allTalentsList = Fu.Skill.GetTalentList(bot)
...
local talentBuildList = Fu.Skill.GetTalentBuild(roleTalentBuildList[role])
local fullSkillBuildList = Fu.Skill.GetSkillList(allAbilitiesList, skillBuildList, allTalentsList, talentBuildList)
local defaultBuild = {
    "item_tango",
    "item_faerie_fire",
    "item_gauntlets",
...
local function ShouldUseOvercharge(ally)
    ...
    return Fu.IsGoingOnSomeone(ally) or attackTarget and attackTarget:GetTeam() == GetOpposingTeam() and isAttacking or #ally:GetNearbyCreeps(200, true) > 2
end
    ...
    for ____, ally in ipairs(allies) do
        do
            ...
            repeat
                ...
                if Fu.IsRetreating(bot) or Fu.GetHP(bot) < 0.25 then
                ...
                if Fu.GetHP(ally) < 0.75 or Fu.GetMP(bot) > 0.8 or HasHealingEffect(bot) or ShouldUseOvercharge(ally) then
                    return BotActionDesire.High, ally
                end
                __continue7 = true
            until true
...
local function considerRelocate()
    if bot:HasModifier("modifier_wisp_tether") and bot.stateTetheredHero ~= nil and (Fu.GetHP(bot.stateTetheredHero) <= 0.2 or Fu.GetHP(bot) <= 0.2) then
        local allyNearbyEnemies = bot.stateTetheredHero:GetNearbyHeroes(1200, true, BotMode.None)
        if #allyNearbyEnemies >= 1 and Fu.GetHP(bot.stateTetheredHero) < Fu.GetHP(allyNearbyEnemies[1]) or #nearbyEnemies >= 1 and Fu.GetHP(bot) < Fu.GetHP(nearbyEnemies[1]) then
            return BotActionDesire.High, GetTeamFountainTpPoint()
        end
    end
    if not bot:HasModifier("modifier_wisp_tether") then
        if #nearbyEnemies >= 1 and Fu.GetHP(bot) < Fu.GetHP(nearbyEnemies[1]) then
            return BotActionDesire.High, GetTeamFountainTpPoint()
        end
    end
    for ____, ally in ipairs(GetUnitList(UnitType.AlliedHeroes)) do
        if IsValidHero(ally) and Fu.IsInTeamFight(ally, 1200) and GetUnitToUnitDistance(bot, ally) > 3000 and ally:WasRecentlyDamagedByAnyHero(2) then
