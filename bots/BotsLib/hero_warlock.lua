...
function X.MinionThink(hMinionUnit)
	...
	if Minion.IsValidUnit( hMinionUnit )
		and ( not Fu.IsKeyWordUnit( 'npc_dota_warlock_minor_imp', hMinionUnit ) )
	then
...
function X.SkillsComplement()
	...
	hEnemyHeroList = Fu.GetNearbyHeroes(bot, 1600, true, BOT_MODE_NONE )
	abilityRef = Fu.IsItemAvailable( "item_refresher" )
...
function X.ConsiderR()
	...
	local hTrueHeroList = Fu.GetEnemyList( bot, 1400 )
	...
	if locationAoE.count >= 1 and Fu.GetHP( bot ) < 0.38 and #hTrueHeroList >= 1
	then
		return BOT_ACTION_DESIRE_LOW, locationAoE.targetloc
	end
	...
	then
		...
		if Fu.IsValidHero( npcTarget )
			...
			and Fu.IsInRange( npcTarget, bot, nCastRange + 200 )
		then
...
function X.ConsiderRFR()
	...
	local manaCost = abilityR:GetManaCost()
	local nRadius = abilityR:GetSpecialValueInt( "aoe" )
	local hTrueHeroList = Fu.GetEnemyList( bot, 1600 )
...
function X.ConsiderE()
	...
	then
		...
		if Fu.IsValidHero( npcTarget )
			...
			and Fu.IsInRange( npcTarget, bot, nCastRange + 200 )
		then
			local enemies = Fu.GetNearbyHeroes(npcTarget, nRadius, false, BOT_MODE_NONE )
			if #enemies >= 2 then
				return BOT_ACTION_DESIRE_HIGH, npcTarget:GetExtrapolatedLocation( nCastPoint )
			end
		end
...
function X.ConsiderW()
	...
	if DotaTime() >= lastCheck + 0.5 then
		local weakest = nil
		local minHP = 100000
		local allies = Fu.GetNearbyHeroes(bot, nCastRange, false, BOT_MODE_NONE )
		if #allies > 0 then
			for i=1, #allies do
				if not allies[i]:HasModifier( "modifier_warlock_shadow_word" )
					and Fu.CanCastOnNonMagicImmune( allies[i] )
...
function X.ConsiderQ()
	...
	then
		local target = Fu.GetVulnerableWeakestUnit( bot, true, true, nCastRange - 100 )
		if target ~= nil and Fu.GetUnitAllyCountAroundEnemyTarget( target, nRadius ) >= 3 then
			return BOT_ACTION_DESIRE_HIGH, target
		end
	end
	...
	then
		...
		if Fu.IsValidHero( target )
			...
			and Fu.IsInRange( target, bot, nCastRange )
		then
			if Fu.GetAroundTargetEnemyUnitCount( target, nRadius ) >= 3 then
				return BOT_ACTION_DESIRE_HIGH, target
			end
		end
	end
