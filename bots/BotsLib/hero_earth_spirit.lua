...
function X.ConsiderBoulderSmash()
	...
	local nInRangeEnemy = Fu.GetNearbyHeroes(bot,nAttackRange, true, BOT_MODE_NONE)
	local nInRangeAlly = Fu.GetNearbyHeroes(bot,1200, false, BOT_MODE_NONE)
	for _, enemyHero in pairs(nInRangeEnemy)
	do
		if Fu.IsValidHero(enemyHero)
		and Fu.CanCastOnNonMagicImmune(enemyHero)
		and Fu.IsInRange(bot, enemyHero, nAttackRange)
	...
	if stoneNearby
	then
		local nEnemyHeroes = Fu.GetNearbyHeroes(bot,math.min(nCastRange, 1600), true, BOT_MODE_NONE)
		local target = Fu.GetCanBeKilledUnit(nEnemyHeroes, nDamage, DAMAGE_TYPE_MAGICAL, false)

		if target ~= nil
		and not Fu.IsSuspiciousIllusion(target)
		then
			local loc = Fu.GetCorrectLoc(target, GetUnitToUnitDistance(bot, target) / nSpeed)
	...
	then
		...
		and Fu.IsInRange(bot, botTarget, math.min(nCastRange, 1600))
		and not Fu.IsSuspiciousIllusion(botTarget)
		then
			local loc = Fu.GetCorrectLoc(botTarget, GetUnitToUnitDistance(bot, botTarget) / nSpeed)
	...
	then
		...
		local target = Fu.GetClosestUnit(nEnemyHeroes)
...
function X.ConsiderRollingBoulder()
	...
	local nEnemyHeroes = Fu.GetNearbyHeroes(bot,math.min(nNearbyEnemySearchRange, 1600), true, BOT_MODE_NONE)
	for _, enemyHero in pairs(nEnemyHeroes)
	do
		if enemyHero:CanBeSeen() and enemyHero:IsChanneling() or Fu.IsCastingUltimateAbility(enemyHero)
		and not Fu.IsSuspiciousIllusion(enemyHero)
		and not bot:HasModifier('modifier_earth_spirit_rolling_boulder_caster')
		then
			local loc = Fu.GetCorrectLoc(enemyHero, (GetUnitToUnitDistance(bot, enemyHero) / nSpeed) + nDelay)
	...
	then
		...
		return BOT_ACTION_DESIRE_HIGH, Fu.Site.GetXUnitsTowardsLocation(bot, loc, nDistance), false
	end
	...
	then
		...
		and not Fu.IsSuspiciousIllusion(target)
		then
			local loc = Fu.GetCorrectLoc(target, (GetUnitToUnitDistance(bot, target) / nSpeed) + nDelay)
	...
	then
		...
		and Fu.CanCastOnNonMagicImmune(botTarget)
		and Fu.IsInRange(bot, botTarget, nDistance2)
		and not Fu.IsSuspiciousIllusion(botTarget)
		...
		then
			...
			and ((#nAllyHeroes >= #nEnemyHeroes) or (#nEnemyHeroes > #nAllyHeroes and Fu.WeAreStronger(bot, nDistance)))
			then
				local loc = Fu.GetCorrectLoc(botTarget, GetUnitToUnitDistance(bot, botTarget) / nSpeed)
		...
		and not botTarget:HasModifier('modifier_faceless_void_chronosphere')
		then
			local loc = Fu.GetCorrectLoc(botTarget, nDelay)
			return BOT_ACTION_DESIRE_HIGH, loc, false
		end
	end
	...
	then
		...
		local loc = Fu.Site.GetXUnitsTowardsLocation(bot, location, nDistance)

		if nAllyHeroes ~= nil and nEnemyHeroes ~= nil
		and ((#nEnemyHeroes > #nAllyHeroes) or (#nAllyHeroes >= #nEnemyHeroes and Fu.GetHP(bot) < 0.45))
		then
			if nStone >= 1
			then
				if Fu.IsInRange(bot, nEnemyHeroes[1], 600)
				then
					return BOT_ACTION_DESIRE_HIGH, loc, true
				else
					return BOT_ACTION_DESIRE_HIGH, loc, false
	...
	then
		...
		then
			local location = Fu.Site.GetXUnitsTowardsLocation(bot, facingFrontLoc, nDistance)
...
function X.ConsiderGeomagneticGrip()
	...
	if Fu.HasAghanimsShard(bot)
	then
		local tableNearbyAllies = Fu.GetNearbyHeroes(bot,nCastRange, false, BOT_MODE_NONE)
		local tableNearbyEnemies = Fu.GetNearbyHeroes(bot,300, false, BOT_MODE_NONE)

		for _, ally in pairs(tableNearbyAllies)
		do
			if Fu.GetHP(ally) < 0.4
			and Fu.IsInRange(bot, ally, nCastRange)
			and not Fu.IsInRange(bot, ally, nCastRange - 250)
	...
	and Fu.IsInRange(bot, target, nCastRange)
	and not Fu.IsSuspiciousIllusion(target)
	then
		local loc = Fu.GetCorrectLoc(target, nCastPoint)
	...
	then
		...
		and not Fu.IsSuspiciousIllusion(botTarget)
		then
			local nTargetAlly  = Fu.GetNearbyHeroes(botTarget, nCastRange, false, BOT_MODE_NONE)
			local nTargetEnemy = Fu.GetNearbyHeroes(botTarget, nCastRange, true, BOT_MODE_NONE)

			if nTargetAlly ~= nil and nTargetEnemy ~= nil
			and #nTargetAlly >= #nTargetEnemy
			then
				local loc = Fu.GetCorrectLoc(botTarget, nCastPoint)
				local isThereStoneNearTarget = IsStoneNearTarget(target)
	...
	then
		...
		then
			...
			do
				...
				and creep:GetHealth() <= nDamage
				then
					local nEnemyHeroesL = Fu.GetNearbyHeroes(bot,1600, true, BOT_MODE_NONE)
