...
function X.ConsiderArcaneAura()
	...
	and Fu.CanCastAbility(ArcaneAura)
	then
		...
		if Fu.IsValidHero( npcTarget )
			...
			and Fu.IsInRange( npcTarget, bot, 500 )
		then
...
function X.ConsiderCombo()
	...
	then
		...
		local amulet = Fu.IsItemAvailable( 'item_shadow_amulet' )
...
function X.ConsiderQ()
	...
	local nAllys =  Fu.GetNearbyHeroes(bot, 1200, false, BOT_MODE_NONE )

	local nEnemysHeroesInRange = Fu.GetNearbyHeroes(bot, nCastRange + nRadius, true, BOT_MODE_NONE )
	local nEnemysHeroesInBonus = Fu.GetNearbyHeroes(bot, nCastRange + nRadius + 150, true, BOT_MODE_NONE )
	local nEnemysHeroesInView = Fu.GetNearbyHeroes(bot, 1600, true, BOT_MODE_NONE )
	...
	if nCanHurtCreepsLocationAoE == nil
		...
		or Fu.GetInLocLaneCreepCount( bot, 1600, nRadius, nCanHurtCreepsLocationAoE.targetloc ) <= 2
	then
	...
	then
		if Fu.IsValid( nWeakestEnemyHeroInBonus )
		then
			local nTargetLocation = Fu.GetCastLocation( bot, nWeakestEnemyHeroInBonus, nCastRange, nRadius )
	...
	then
		...
		--对进攻目标使用
		local npcEnemy = Fu.GetProperTarget( bot )
		...
		then
			...
			then
				local nTargetLocation = Fu.GetCastLocation( bot, npcEnemy, nCastRange, nRadius )
				if nTargetLocation ~= nil
				then
					return BOT_ACTION_DESIRE_HIGH, nTargetLocation
				end
		...
		--无敌人时清理兵线
		if 	Fu.IsValid( nEnemysWeakestLaneCreeps2 )
		...
		if nCanKillCreepsLocationAoE.count >= 3
			and ( Fu.IsValid( nEnemysWeakestLaneCreeps1 ) or nLV >= 25 )
			and #nEnemysHeroesInBonus <= 0
			and bot:GetActiveMode() ~= BOT_MODE_ATTACK
			and nSkillLV >= 3
		then
	...
	then
		...
		if nCanKillCreepsLocationAoE.count >= 2
			and Fu.IsValid( nEnemysWeakestLaneCreeps1 )
		then
			return BOT_ACTION_DESIRE_HIGH, nCanKillCreepsLocationAoE.targetloc
		end
	...
	if #nAllys <= 2 and nSkillLV >= 3
		and ( Fu.IsPushing( bot ) or Fu.IsDefending( bot ) )
	then
	...
	if Fu.IsValid( nNeutarlCreeps[1] )
	then
		...
		do
			...
			then
				local nTargetLocation = Fu.GetCastLocation( bot, creep, nCastRange, nRadius )
				if nTargetLocation ~= nil
				then
					return BOT_ACTION_DESIRE_HIGH, nTargetLocation
				end
...
function X.ConsiderW()
	...
	local nEnemysHeroesInRange = Fu.GetNearbyHeroes(bot, nCastRange, true, BOT_MODE_NONE )
	local nEnemysHeroesInBonus = Fu.GetNearbyHeroes(bot, nCastRange + 200, true, BOT_MODE_NONE )
	...
	--击杀敌人
	if Fu.IsValid( nWeakestEnemyHeroInRange )
		and Fu.CanCastOnTargetAdvanced( nWeakestEnemyHeroInRange )
	then
		if Fu.WillMagicKillTarget( bot, nWeakestEnemyHeroInRange, nDamage, nCastPoint )
		then
	...
	then
		...
		if Fu.IsValid( nEnemysHeroesInView[1] )
		then
			if Fu.GetAllyUnitCountAroundEnemyTarget( bot, nEnemysHeroesInView[1], 350 ) >= 5
				and not Fu.IsDisabled( nEnemysHeroesInView[1] )
	...
	then
		...
		do
			if Fu.IsValid( EnemyplayerCreep )
	...
	then
		...
		--先远
		if Fu.IsValid( nEnemysStrongestCreeps2 )
			and ( DotaTime() > 10 * 60
		...
		--再近
		if Fu.IsValid( nEnemysStrongestCreeps1 )
			and ( DotaTime() > 10 * 60
	...
	then
		...
		do
			if Fu.IsValid( npcEnemy )
				...
				and Fu.CanCastOnTargetAdvanced( npcEnemy )
				and not Fu.IsDisabled( npcEnemy )
				and Fu.IsInRange( npcEnemy, bot, nCastRange - 80 )
			then
				return BOT_ACTION_DESIRE_HIGH, npcEnemy
			end
		end
...
function X.ConsiderR()
	...
	local nEnemysHeroesInRange = Fu.GetNearbyHeroes(bot, nRadius, true, BOT_MODE_NONE )
	local nWeakestEnemyHeroInRange, nWeakestEnemyHeroHealth1 = X.cm_GetWeakestUnit( nEnemysHeroesInRange )
	...
	do
		if Fu.IsValid( enemy )
			...
			and ( Fu.IsDisabled( enemy )
				  or Fu.IsInRange( bot, enemy, nRadius * 0.82 - enemy:GetCurrentMovementSpeed() ) )
		then
	...
	then
		...
		if Fu.IsValidHero( npcTarget )
			and Fu.CanCastOnNonMagicImmune( npcTarget )
			and ( Fu.IsDisabled( npcTarget ) or Fu.IsInRange( bot, npcTarget, 280 ) )
	...
	if Fu.IsRetreating( bot ) and nHP > 0.38
	then
		local nEnemysHeroesNearby = Fu.GetNearbyHeroes(bot, 500, true, BOT_MODE_NONE )
		local nEnemysHeroesFurther = Fu.GetNearbyHeroes(bot, 1300, true, BOT_MODE_NONE )
...
function X.ConsiderCrystalClone()
	...
	then
		...
		then
			...
			then
				return BOT_ACTION_DESIRE_HIGH, Fu.Site.GetXUnitsTowardsLocation(bot, Fu.GetTeamFountain(), nRadius)
			end
		end
	end
...
function X.cm_GetWeakestUnit( nEnemyUnits )
	...
	do
		if 	Fu.CanCastOnNonMagicImmune( unit )
		then
...
function X.cm_GetStrongestUnit( nEnemyUnits )
	...
	do
		if 	unit ~= nil and unit:IsAlive()
			...
			and not Fu.IsOtherAllysTarget( unit )
