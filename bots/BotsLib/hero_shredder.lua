...
function X.ConsiderTimberChain()
	...
	then
		...
		and not Fu.AreTreesBetween(bot, botTarget:GetLocation(), nRadius)
		and nInRangeAlly ~= nil and nInRangeEnemy ~= nil
		and #nInRangeAlly >= #nInRangeEnemy
		then
			local nDelay = (GetUnitToUnitDistance(bot, botTarget) / nSpeed) + nCastPoint
			local nTargetTree = Fu.GetBestTree(bot, botTarget:GetExtrapolatedLocation(nDelay), botTarget, nCastRange, nRadius)

			if nTargetTree ~= nil
			then
				if bot:GetLevel() < 6
				and WhirlingDeath:IsTrained()
				and not Fu.CanKillTarget(botTarget, nDamage + nWhirlingDamage, DAMAGE_TYPE_PURE)
				then
	...
	then
		...
		then
			local nTargetTree = Fu.GetBestRetreatTree(bot, nCastRange)
...
function X.ConsiderChakram()
	...
	local nCastRange = Fu.GetProperCastRange(false, bot, Chakram:GetCastRange())
	...
	then
		...
		and #nInRangeAlly >= #nInRangeEnemy
		then
			local loc = Fu.GetUltLoc(bot, botTarget, nManaCost, nCastRange, nSpeed)
...
function X.ConsiderTwistedChakram()
	if not Fu.CanCastAbility(TwistedChakram) then
		return BOT_ACTION_DESIRE_NONE, 0
	end

	local nCastRange = Fu.GetProperCastRange(false, bot, TwistedChakram:GetCastRange())
	...
	then
		for _, enemy in pairs(tEnemyHeroes) do
			...
			and Fu.IsInRange(bot, enemy, 500)
	...
	if (Fu.IsDefending(bot) or Fu.IsPushing(bot)) and nManaAfter > 0.3
	then
		if Fu.IsValid(tEnemyLaneCreeps[1])
		and Fu.CanBeAttacked(tEnemyLaneCreeps[1])
		and not Fu.IsRunning(tEnemyLaneCreeps[1]) then
	...
	if Fu.IsFarming(bot) and nManaAfter > 0.3
	then
...
-- 	local nCastRange = Fu.GetProperCastRange(false, bot, Chakram2:GetCastRange())
...
function X.ConsiderFlamethrower()
	...
	then
		...
		if Fu.IsValidTarget(botTarget)
		and Fu.CanCastOnNonMagicImmune(botTarget)
		and Fu.IsInRange(bot, botTarget, nFrontRange)
