...
function X.ConsiderW()
	...
	if Fu.IsWithoutTarget( bot ) and Fu.GetAttackProjectileDamageByRange( bot, 1600 ) >= bot:GetHealth()
	then
		return BOT_ACTION_DESIRE_MODERATE
	end
	...
	then
		...
		if Fu.IsValidHero( npcTarget )
			...
			and Fu.IsInRange( npcTarget, bot, nRadius - 30 )
			and ( Fu.GetHP( bot ) < 0.25
				or ( not Fu.IsInRange( npcTarget, bot, 430 )
					  and bot:IsFacingLocation( npcTarget:GetLocation(), 30 )
					  and not npcTarget:IsFacingLocation( bot:GetLocation(), 120 ) ) )
		then
			local targetAllies = Fu.GetNearbyHeroes(npcTarget, 1000, false, BOT_MODE_NONE )
...
function X.ConsiderQ()
	...
	if not abilityQ:IsFullyCastable()
		or bot:IsInvisible()
		or ( Fu.GetHP( bot ) > 0.62 and abilityR:GetCooldownTimeRemaining() < 6 and bot:GetMana() < abilityR:GetManaCost() )
	then
		return BOT_ACTION_DESIRE_NONE
	end
	...
	if Fu.IsRetreating( bot ) or Fu.GetHP( bot ) < 0.5
	then
		local tableNearbyEnemyHeroes = Fu.GetNearbyHeroes(bot, 2 * nRadius, true, BOT_MODE_NONE )
		if ( bot:WasRecentlyDamagedByAnyHero( 2.0 ) and #tableNearbyEnemyHeroes > 0 )
			or ( Fu.GetHP( bot ) < 0.75 and bot:DistanceFromFountain() < 400 )
			or Fu.GetHP( bot ) < Fu.GetMP( bot )
			or Fu.GetHP( bot ) < 0.25
		then
			return BOT_ACTION_DESIRE_MODERATE
		end
	end
	...
	then
		...
		do
			...
			then
				...
				if Fu.CanKillTarget( creep, nDamage + 2, nDamageType )
				then
		...
		if nCanKillCount >= 2
			or ( nCanKillCount >= 1 and Fu.GetMP( bot ) > 0.9 )
			or ( nCanHurtCount >= 3 and bot:GetActiveMode() ~= BOT_MODE_LANING )
			or ( nCanHurtCount >= 3 and nCanKillCount >= 1 and Fu.IsAllowedToSpam( bot, 190 ) )
			or ( nCanHurtCount >= 3 and Fu.GetMP( bot ) > 0.8 and bot:GetLevel() > 10 and #tableNearbyEnemyCreeps == 3 )
	...
	then
		local tableNearbyEnemyHeroes = Fu.GetNearbyHeroes(bot, nRadius, true, BOT_MODE_NONE )
		local tableNearbyAlliesHeroes = Fu.GetNearbyHeroes(bot, nRadius, false, BOT_MODE_NONE )
		local lowHPAllies = 0
...
function X.ConsiderR()
	...
	then
		...
		then
			...
			if Fu.CanKillTarget( botTarget, EstDamage, DAMAGE_TYPE_MAGICAL )
	...
	then
		...
		do
			...
			then
				...
				if Fu.CanKillTarget( npcEnemy, EstDamage, DAMAGE_TYPE_MAGICAL )
				then
					npcToKill = npcEnemy
					break
				end
	...
	do
		...
		then
			local EstDamage = X.GetEstDamage( bot, npcEnemy, nDamagePerHealth )
			if Fu.CanCastOnNonMagicImmune( npcEnemy ) and Fu.CanKillTarget( npcEnemy, EstDamage, DAMAGE_TYPE_MAGICAL )
			then
...
function X.GetEstDamage( bot, npcTarget, nDamagePerHealth )
	...
	local AroundTargetAllyCount = Fu.GetAroundTargetAllyHeroCount( npcTarget, 650 )
	...
	if bot:HasScepter() and Fu.GetHP( bot ) < 0.2 then EstDamage = EstDamage + targetMaxHealth * 0.3 end
	...
	then
		...
		do
			if Fu.IsValid( npcEnemy )
				...
				and Fu.CanCastOnTargetAdvanced( npcEnemy )
			then
				local enemyHeroList = Fu.GetNearbyHeroes(npcEnemy,  500, false, BOT_MODE_NONE )
				local allyHeroList = Fu.GetNearbyHeroes(npcEnemy,  500, true, BOT_MODE_NONE )
